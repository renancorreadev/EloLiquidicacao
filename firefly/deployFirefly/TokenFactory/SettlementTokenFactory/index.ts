...
import { SettlementTokenFactory__factory } from '../../../../src/types/factories/contracts/base/Factory/SettlementTokenFactory__factory';
...
const contractByteCode = SettlementTokenFactory__factory.bytecode;
const contractABI = SettlementTokenFactory__factory.abi;

export class DeploySettlementTokenFactory {
    host: string;
    host_port: string;
    ...
    async makeDeployRequest(): Promise<string> {
        ...
        try {
            ...
            if (contractTx) {
                console.log("Contrato SettlementTokenFactory - Deployado com sucesso!");
            }

            return contractTx;
        } catch (error: any) {
            const errorMessage = error.response ? error.response.data : error.message;
            throw new Error(`Erro ao fazer a solicitação de deploy SettlementTokenFactory: ${JSON.stringify(errorMessage)}`);
        }
    }
